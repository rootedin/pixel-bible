...
## 교안·활동지 인쇄 (수업 준비용)
- 레슨 주소 뒤에 `?guide` → **교사 교안**: 장면 그림·대사·말씀·퀴즈 정답과 해설·나눔 질문이 한 장에 펼쳐진다.
- `?sheet` → **학생 활동지**: 정답을 뺀 퀴즈, 빈칸 채우기(낱말 보기 포함), 생각해 보기, 암송 구절.
- 두 화면 모두 브라우저 인쇄(Ctrl+P)로 그대로 뽑도록 인쇄 CSS가 들어 있다. 상단 `▤ 교안` 버튼으로도 열린다.
- 장면에 `notes: '교사용 메모'` 를 넣으면 교안에만 보이고 수업 화면에는 나오지 않는다.
