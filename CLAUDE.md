...
- 구성: 도입(빈 월드/질문) → 본문 장면 → 핵심 말씀(verse) → 퀴즈 2개 → 나눔 질문 2개.
- **수업 시간은 강당 20분.** 장면당 1분 안팎(교사가 한두 문장 덧붙이는 시간 포함)으로 보고 **전체 24~28장면**을 목표로 한다.
  분량은 장면 수로 채우고, 한 장면의 줄 수를 늘리지 않는다 (3줄 이하 유지). 시대 배경·앞뒤 사건·책(교재)의 설명은 별도 장면으로 넣는다.
- 교재 원문이 주어지면 그 이야기 흐름(인물의 마음·배경 설명·주제)을 따르되, 성경 본문에 없는 내용은 대사가 아닌 내레이션으로 다룬다.
...
- **분량 상한** (lint.js 가 검사한다)
  - 장면당 `lines` 는 **3줄 이하**. 더 필요하면 장면을 나누거나 문장을 합친다.
  - 말씀 카드(`verse`)는 **레슨당 5장 이하**. 핵심 구절에만 달고, 대사로 이미 읽은 구절은 카드로 반복하지 않는다.
  - 퀴즈 보기는 길이를 비슷하게. 정답만 길면 답이 보인다 (정답이 최장 오답의 1.5배를 넘지 않게).
  - 나눔 질문은 예/아니요로 끝나는 수사의문문 대신 열린 질문으로 ("무엇입니까?", "어디입니까?").
- **하나님·예수님·천사의 대사는 킹흠정역 표현 그대로** 쓴다. 풀어 쓰지 말고 `node verse.js` 출력을 옮긴다
  (lint 가 낱말 연쇄 일치율로 검사해 50% 미만이면 경고). **성경을 그대로 인용한 대사는 55자를 넘어도 줄이지 않는다.**
- **용어 통일** (lint 가 잡는다): 이집트(애굽 ✕), 침례·침례자 요한(세례 ✕), 재판관(사사 ✕), 대언자(선지자 ✕),
  가자(가사 ✕), 카이사르(가이사 ✕), 다마스쿠스(다메섹 ✕), 바빌론(바벨론 ✕), 아시리아(앗수르 ✕).
...
- 동물/사물: sheep cow donkey lion snake fish bird dove sun moon star heart cross fire tablets
  scroll bread fruit flower flower2 tuft torch stone jar coin basket reed frog brick fly locust chariot manna ark censer cord horn spear smoke sword fleece fox sheaf honeycomb harp arrow basket(갈대 궤+아기) reed(부들/갈대) camel gem yoke grapes axe ship
...
- 상호작용(걷기·찾기·빈칸·퀴즈·선택)이 끝나지 않으면 화면/대화창 클릭이나 Space·Enter·→ 로 건너뛰어지지 않는다.
  막히면 교사가 프레젠터 `PageDown` 또는 `다음 ▶` 버튼으로 언제든 넘길 수 있다.
...
## 품질 점검 (레슨을 고쳤으면)
1. `node lint.js` — 오류 0 을 유지한다. 경고는 위 "내용 작성 원칙"의 분량·말투·용어·퀴즈 규칙이다.
2. `node snap.js` — 장면 수·상호작용 구성이 바뀌었는지 비교. 의도한 변경이면 `node snap.js --update`.
3. `node genlist.js` → `node build.js` 로 목록과 `dist/` 재생성.
- 퀴즈 보기는 무작위 순서로 섞여 나온다. "위의 모든 것", "①과 ②" 처럼 **순서에 기대는 보기는 쓰지 않는다**
  (순서를 고정해야 하면 `quiz: { shuffle: false, ... }`).
