...
- **수업 시간은 강당 20분.** 장면당 1분 안팎(교사가 한두 문장 덧붙이는 시간 포함)으로 보고 **전체 24~28장면**을 목표로 한다.
  분량은 장면 수로 채우고, 한 장면의 줄 수를 늘리지 않는다 (3줄 이하 유지). 시대 배경·앞뒤 사건·책(교재)의 설명은 별도 장면이나 `notes` 로 넣는다.
- 교재 원문이 주어지면 그 이야기 흐름(인물의 마음·배경 설명·주제)을 따르되, 성경 본문에 없는 내용은 대사가 아닌 내레이션·`notes` 로 다룬다.
